...
const {
  disabled, info, infoExtra, morningDisabled, nightDisabled, cnt
} = s;

export const main  = `<div class="${s.monthsWrapper}"></div>`;

export const month = `
  <div class="${s.mCell} js-month">
...
export const weekDay = (label, dis, disStart, isOutAvailable, rate, minStay, rateT, minStayT, forceDaysEnabled) => `
  <td ${forceDaysEnabled || !dis ? 'data-enabled' : 'data-disabled'}
       ${forceDaysEnabled || isOutAvailable ? 'data-available-out' : ''}
       data-value="${label}"
...
export const forward = `
  <button class="${s.forward}" role="button">
...
export const back = `
  <button class="${s.back}" role="button">
    <svg viewBox="0 0 1000 1000" width="20" height="20">
